...
describe('SearchReducer', () => {
    ...
    it('should query the search results and update the state', () => {
        let state = fromSearch.reducer(initialState, SearchActions.searchQuery({query: 'angular'}));
        expect(state.searchQuery).toEqual('angular');

        state = fromSearch.reducer(initialState, SearchActions.searchResults({results: {kind: 'volume', totalItems: 100, items: mockBooks}}));
